...
filter_pattern.W_Sequence <- function(w_sequence,
  ...
  pattern <- map(w_sequence, function(w_sequence_itemset) {
...
format_sequence.W_Sequence_Pattern <-
  function(w_sequence_pattern, html_format = FALSE) {
    ...
    if (html_format) {
      if(n > 1){
        colors <- rev(colormap::colormap(colormap = "bluered", nshades = n) %>% stringr::str_sub(1, -3))
      } else {
        colors <- colormap::colormap(nshades = 2)[1]
      }

      # cuts <- floor(n*seq(0,1,0.2))[2:5]
      w_sequence_pattern %>% map_chr(function(w_itemset) {
        tibble(
      ...
      w_sequence_pattern %>% map_chr(function(w_itemset) {
          if (length(w_itemset$elements) > 0) {
            str_c(w_itemset$elements, collapse = ", ") %>% paste0("(", ., ")")
          } else {
            NA
          }
        }) %>% .[!is.na(.)] %>%
...
format_sequence.W_Sequence_Pattern_List <- function(w_sequence_pattern_list, ...) {
  ...
  map_chr(w_sequence_pattern_list,
          function(w_sequence_pattern) {
            format_sequence(w_sequence_pattern, ...)
    })
  }
